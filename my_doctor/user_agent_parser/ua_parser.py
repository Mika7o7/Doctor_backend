...
def ParseUserAgent(user_agent_string, **jsParseBits):
    ...
    """
    return _cached(
        user_agent_string, jsParseBits, "user_agent", _ParseUserAgent
    )
...
def _ParseUserAgent(user_agent_string, jsParseBits):
    if jsParseBits:
        warnings.warn(
            "javascript overrides are deprecated and \
...
def _ParseOS(user_agent_string, jsParseBits):
    if jsParseBits:
        warnings.warn(
            "javascript overrides are deprecated \
